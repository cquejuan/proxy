...
function FindProxyForURL(url, host) {
    ...
    if(whitelistTLD(TLD) == true){
        return direct
    }

    if(searchWhitelistDB(SUB, SLD, TLD) == true){
        return direct
    }
...
function searchWhitelistDB(SUB, SLD, TLD){
            ...
            for(j = 0; j < item.z.length; j++){
                if(item.z[j] == TLD){
                    for(k = 0; item.x.length; k++){
                        if(item.x[k] == "*" || item.x[k] == "" || item.x[k] == SUB){
                            return true
                        }
                    }
                }
